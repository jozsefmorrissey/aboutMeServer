...
case "$1" in
	'')
		...
		if [ "$(ls -A /u01/app/oracle/oradata 2>/dev/null)" ]; then
			...
			set +e
			cp -r /u01/app/oracle-product/12.1.0/xe/dbs /u01/app/oracle/dbs
			set -e
...
root@9e04281528de:/# sed "s/mv \(\/u01\/app\/oracle-product\/12.1.0\/xe\/dbs \/u01\/app\/oracle\/dbs\)/cp -f \1/g" ./entrypoint.sh
#!/bin/bash
set -e

# Prevent owner issues on mounted folders, if desired
ENABLE_OWNERSHIP_FIX=true
echo $ENABLE_OWNERSHIP_FIX
if [ "$ENABLE_OWNERSHIP_FIX" = true ] ; then
	chown -R oracle:dba /u01/app/oracle
	chown -R oracle:dba /docker-entrypoint-initdb.d/
fi
...
case "$1" in
	'')
		...
		if [ "$(ls -A /u01/app/oracle/oradata 2>/dev/null)" ]; then
			...
			#printf "Setting up:\nprocesses=$processes\nsessions=$sessions\ntransactions=$transactions\n"
			set +e
			cp -f /u01/app/oracle-product/12.1.0/xe/dbs /u01/app/oracle/dbs
			set -e
